...
export default class AdminProductView extends Component {
  ...
  onDelete = (id) => {
    axios.delete(`http://localhost:5000/posts1/delete/${id}`).then((res) => {
  ...
  filterData(posts1, searchKey) {
    const result = posts1.filter((post) =>
    ...
    return (
      ...
      <NavBarpic />
          ...
          <tbody>
            {this.state.posts1.map((posts1, index) => (
              <tr>
                ...
                <td><img src={`http://localhost:5000/public/${posts1?.photo}`} alt={posts1?.pname} style={{ objectFit: 'contain', width: 100, height:100 }}/></td>
               
                

                <td>
                  <a className="btn btn-warning" href={`/admin-editproduct/${posts1._id}`}>
                  ...
                  <a
                    ...
                    href="#"
                    onClick={() => this.onDelete(posts1._id)}
                  >
        ...
        <button className="btn btn-success">
          <a href="/admin-addproduct" style={{ textDecoration: "none", color: "white" }}>
            Add Products
          </a>
        </button>
